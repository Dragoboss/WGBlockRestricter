...
public class BlockListener implements Listener{
	...
	public void onPlayerInteract(PlayerInteractEvent e){
		if(e.getAction() == Action.RIGHT_CLICK_BLOCK && e.hasItem() && Utils.baseMaterials.containsKey(e.getItem().getType()) && !e.getPlayer().hasPermission("wgblockrestricter.ignore") && !Utils.placeAllowedAtLocation(wgPlugin, e.getMaterial(), e.getClickedBlock().getRelative(e.getBlockFace()).getLocation())){
	...
	public void onBlockPlace(BlockPlaceEvent e){
		if(!e.getPlayer().hasPermission("wgblockrestricter.ignore") && !Utils.placeAllowedAtLocation(wgPlugin, e.getBlockPlaced().getType(), e.getBlockPlaced().getLocation())){
			String message = plugin.getConfig().getString("messages.deny-block-place", "&cYou are not allowed to place {block} here.");
			if(!"".equals(message)){
	...
	public void onBlockBreak(BlockBreakEvent e){
		if(!e.getPlayer().hasPermission("wgblockrestricter.ignore") && !Utils.breakAllowedAtLocation(wgPlugin, e.getBlock().getType(), e.getBlock().getLocation())){
	...
	public void onHangingPlace(HangingPlaceEvent e){
		...
		if(!e.getPlayer().hasPermission("wgblockrestricter.ignore") && !Utils.placeAllowedAtLocation(wgPlugin, mat, e.getBlock().getRelative(e.getBlockFace()).getLocation())){
	...
	public void onHangingBreakByEntity(HangingBreakByEntityEvent e){
		if(e.getRemover() instanceof Player){
			...
			if(!player.hasPermission("wgblockrestricter.ignore") && !Utils.breakAllowedAtLocation(wgPlugin, mat, e.getEntity().getLocation())){
